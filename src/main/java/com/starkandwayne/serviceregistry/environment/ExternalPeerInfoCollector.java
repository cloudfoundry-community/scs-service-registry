...
class ExternalPeerInfoCollector implements InitializingBean {
   ...
   private final DynamicCertificateTruster certificateTruster;
   ...
   public ExternalPeerInfoCollector(ConfigurableEnvironment environment, ObjectMapper objectMapper, RestOperations restTemplate, PeerEnvironmentManager environmentManager, DynamicCertificateTruster certificateTruster, RefreshScope refreshScope) {
   ...
   public void afterPropertiesSet() throws Exception {
      ...
      if (!StringUtils.isEmpty(peersSetByBrokerString)) {
         List peersSetByBroker = (List)this.objectMapper.readValue(peersSetByBrokerString, new TypeReference() {
         });
         ...
         peersSetByBroker.forEach((peerConfig) -> {
            this.peerInfoCompletionService.submit(null, () -> {
   ...
   private void trustPeerCertificates(PeerConfigUserInput peerConfig) throws InterruptedException {
      ...
      while(true) {
         ...
         try {
            this.certificateTruster.trustCertificate(uri.getHost(), port);
